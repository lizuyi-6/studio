...
import { Hexagon, Layers, Box, Cpu, Menu, X, ArrowRight, Github, Globe, Mail, Zap, Compass, GitBranch, Terminal, Figma, Plus, Database, Layout, Server, Smartphone, Code, Braces, Cloud, Command, Settings } from 'lucide-react';
...
const SectionHeader = ({ title, subtitle }) => (
    <div className="text-center mb-24">
        <h1 className="text-5xl md:text-7xl font-bold mb-8 tracking-tight">
            {title} <span className="text-[#0066FF]">{subtitle}</span>
        </h1>
        <div className="w-24 h-1 bg-[#0066FF] mx-auto rounded-full"></div>
    </div>
);
...
const TechCard = ({ title, icon, items, className = "" }) => (
    <div
        style={{ border: '2px solid red' }} // DEBUG: Force visibility
    ...
    >
        <h1 style={{ color: 'red', fontSize: '20px' }}>DEBUG: CARD RENDERED</h1>
...
const TechView = () => {
    return (
        <div className="pt-32 pb-20 min-h-screen container mx-auto px-6">
            ...
            {/* Bento Grid Layout */}
            {/* Bento Grid Layout */}
            <h1 style={{ color: 'red', fontSize: '40px', zIndex: 9999, position: 'relative' }}>DEBUG: GRID START</h1>
            <div
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="flex gap-3">
                                        <a href="https://github.com/lizuyi-6" target="_blank" className="hover:text-[#0066FF] transition-colors"><Github size={20} /></a>
                                        <a href="https://aetherstudio.top/" target="_blank" className="hover:text-[#0066FF] transition-colors"><Globe size={20} /></a>
                                    </div>
                                </div>
...
export default function App() {
    ...
    useEffect(() => {
        const views = ['home', 'solutions', 'tech', 'about'];
        // 使用 useRef 来保持 lastScrollTime，避免因闭包导致的不更新问题，或者更简单地放在外部变量（但不推荐全局）
        // 这里为了简单，我们使用局部变量配合 useEffect 依赖
        // 但更好的方式是把 cooldown 检查放在 state update 之外

        // 其实 handleWheel 每次 render 都会重置 lastScrollTime 如果定义在这里。
        // 所以 lastScrollTime 必须是 ref。
    }, []);
